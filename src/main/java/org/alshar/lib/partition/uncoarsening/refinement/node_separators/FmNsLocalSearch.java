...
public class FmNsLocalSearch {

    private List<Integer> movedNodes = new ArrayList<>();

    public FmNsLocalSearch() {
    }

    public int performRefinement(PartitionConfig config, GraphAccess G, boolean balance, int to) {
        ...
        if (startNodes.isEmpty()) return 0;
        ...
        while (stepsTillLastImprovement < config.getSepFmUnsuccSteps()) {
            ...
            if (balance) {
                ...
                if (gainToA == gainToB) {
                    ...
                    topGain = gainToA > gainToB ? gainToA : gainToB;
                    toBlock = topGain == gainToA ? 0 : 1;
                }
            }

            long otherGain = gainToA > gainToB ? gainToB : gainToA;
        ...
        return (int) inputSeparator - (int) bestSeparator;
    }
    ...
    public int performRefinement(PartitionConfig config, GraphAccess G, List<Integer> blockWeights,
        ...
        List<Integer> startNodes = new ArrayList<>();
        separator.forAllBoundaryNodes(startNodes::add);  // Collect boundary nodes into startNodes list
        ...
        int bestSeparator = blockWeights.get(2);
        int inputSeparator = blockWeights.get(2);
        int bestDiff = Math.abs((int) (blockWeights.get(1) - blockWeights.get(0)));
        int undoIdx = 0;
    ...
    private void moveNode(GraphAccess G, int node, int toBlock, int otherBlock,
                          List<Long> blockWeights, List<Boolean> movedOutOfS,
                          List<MaxNodeHeap> queues, List<ChangeSet> rollbackInfo) {
    ...
    private void moveNode(GraphAccess G, int node, int toBlock, int otherBlock,
                          List<Integer> blockWeights, List<Boolean> movedOutOfS,
                          List<MaxNodeHeap> queues, List<ChangeSet> rollbackInfo,
